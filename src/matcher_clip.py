"""
CLIP-only Matcher.
Uses raw CLIP visual similarity for species matching without LLM fusion.
"""
...
class CLIPMatcher:
    """Match species using pure CLIP visual embeddings."""
    ...
    def compute_scores(
        ...
        for species_name, species_info in knowledge_base.items():
            ...
            if isinstance(species_info, dict):
                ...
                full_text = f"{self.clip_prefix}{species_name}. "
                if appearance:
        ...
        species_embeddings = self.clip.encode_text(species_texts, normalize=True)
        ...
        for species_name, species_embedding in zip(species_names, species_embeddings):
            similarity = self.clip.compute_similarity(
                image_embedding, species_embedding
            )
    ...
    def match(self, image_path: str, knowledge_base: Dict[str, Dict]) -> Dict:
        """
        Perform species matching using CLIP similarity only.
