"""
BLIP-LLM Fusion Matcher.
Combines visual BLIP scores with textual LLM scores for species matching.
"""
...
class BLIPLLMFusionMatcher:
    """Match species using fusion of BLIP visual and LLM textual scores."""
    ...
    def __init__(
        ...
        """
        Initialize BLIP-LLM Fusion Matcher.
    ...
    def compute_visual_scores(
        self, image_path: str, blip_kb: Dict[str, np.ndarray]
    ) -> Dict[str, float]:
        """
        ...
        Args:
            ...
            blip_kb: Dictionary of species -> BLIP text embeddings
        ...
        visual_scores = {}
        for species, text_embedding in blip_kb.items():
            similarity = self.blip.compute_similarity(image_embedding, text_embedding)
    ...
    def fuse_scores(
        ...
        Args:
            visual_scores: BLIP-based visual scores
    ...
    def predict_species(
        ...
        """
        Predict species using BLIP-LLM fusion.
        ...
        """
        # Prepare BLIP KB (cache if not done)
        if self.blip_kb_cache is None:
            self.blip_kb_cache = self.blip.encode_knowledge_base(
                knowledge_base, prefix=self.blip_prefix
            )
        ...
        visual_scores = self.compute_visual_scores(image_path, self.blip_kb_cache)
