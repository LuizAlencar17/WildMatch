"""
Main entry point for WildMatch-BLIP pipeline.
Pure BLIP visual similarity for zero-shot species classification.
"""
...
from pipelines.wildmatch_blip import WildMatchBLIP
...
def main(dataset="serengeti", image_type="full"):
    ...
    print("=" * 70)
    print("WildMatch-BLIP Pipeline")
    print("Zero-shot classification using pure BLIP visual embeddings")
    ...
    blip_prefix = "a camera trap image of an animal. "
    ...
    output_path = (
        f"results/predictions/{dataset}_{image_type}_blip_predictions.csv"
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="WildMatch-BLIP Pipeline")
    parser.add_argument(
        "--dataset",
        type=str,
        default="serengeti",
